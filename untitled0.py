...
# Comando mágico para mostrar os gráficos diretamente no notebook
# %matplotlib inline
...
# Carregar os dados, já convertendo as colunas de data
...
# Calcula a participação individual e acumulada
...
# Exibir o resultado da análise ABC
df_abc

fig, ax1 = plt.subplots(figsize=(10, 6))

# Gráfico de barras para o faturamento por categoria
...
# Título
fig.suptitle('Análise de Pareto por Categoria de Produtos', fontsize=16)
plt.show()
...
# Agrupa para obter as médias por categoria
...
# Conta as peças
estoque_atual = df[df['Status'] == 'Disponível']['Categoria'].value_counts().reset_index(name='Pecas_Estoque')
...
# Exibe a tabela de insights estratégicos
df_desempenho

fig, ax = plt.subplots(figsize=(12, 8))

# O tamanho da bolha (s) pode representar a margem de lucro, e a cor (c) também
scatter = ax.scatter(
    df_desempenho['Giro_Medio_Dias'],
    df_desempenho['Pecas_Vendidas'],
    s=df_desempenho['Margem_%'] * 8, # Multiplicador para as bolhas ficarem visíveis
    c=df_desempenho['Margem_%'],
    cmap='viridis', # Escala de cores
    alpha=0.7
)

# Adicionar rótulos para cada categoria
...
# Linhas de média para dividir em quadrantes
...
class AppGrafico(tk.Tk):
    """
    Classe principal da aplicação que herda de tk.Tk para criar a janela principal.
    """
    def __init__(self):
        super().__init__()

        # --- 1. Configuração da Janela Principal ---
        self.title("Gráfico Matplotlib no Tkinter")
        ...
        self.botao_atualizar = ttk.Button(
            ...
            command=self.desenhar_grafico # Ação que o botão executa
        )
        ...
        # Variável para armazenar a referência do canvas do gráfico
        self.canvas = None

        # --- 3. Desenhar o Gráfico Inicial ---
        self.desenhar_grafico()

    def desenhar_grafico(self):
        """
        Função para gerar e desenhar o gráfico de barras na interface.
        """
        # --- A. Limpeza do Gráfico Anterior (IMPORTANTE!) ---
        # Se um canvas já existe, ele é destruído para dar lugar ao novo.
        # Isso evita que múltiplos gráficos sejam sobrepostos.
        if self.canvas:
            self.canvas.get_tk_widget().destroy()

        # --- B. Geração de Dados Fictícios ---
        categorias = ['Vendas', 'Marketing', 'TI', 'RH', 'Logística']
        valores = np.random.randint(low=10, high=100, size=len(categorias))

        # --- C. Criação da Figura e do Gráfico Matplotlib ---
        # 1. Cria uma figura Matplotlib. O 'dpi' controla a resolução.
        figura = Figure(figsize=(5, 4), dpi=100)

        # 2. Adiciona um subplot (eixo/área de desenho) à figura.
        # '111' significa uma grade de 1x1, primeiro subplot.
        eixo = figura.add_subplot(111)

        # 3. Cria o gráfico de barras no eixo definido.
        ...
        figura.tight_layout() # Ajusta o gráfico para caber na figura

        # --- D. Incorporação do Gráfico no Tkinter ---
        ...
        # (Opcional) Adiciona a barra de ferramentas de navegação do Matplotlib
